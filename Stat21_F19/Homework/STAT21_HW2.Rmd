...
**Q 1)** Create a non-normal data set of sample size $n=20$ by setting a random seed and drawing your observations from a Beta distribution with \textcolor{red}{$\alpha=5$} and $\beta=1.23$.(5 points)  
  ...
  c) Create $24$ other data sets each of sample size $n=20$, and each also drawn from a Beta distribution with \textcolor{red}{$\alpha=5$} and $\beta=1.23$. \textcolor{red}{It might be useful to create a matrix of size $20\times 25$ and then break this apart into individual data sets, just make sure the end result is a *tibble*.} For each of these $25$ \textcolor{red}{data sets}, compute the mean \textcolor{red}{(check out the *colMeans* function)} and store these $25$ means in a new tibble called *beta_means*. Print the first few rows of this tibble, *beta_means*. 
...
Hint: The natural logarithm function in base R is simply \textcolor{red}{*log()*}.
