...
**Q 3)** How does the height (Y) of a skyscraper depend on the number of stories \textcolor{red}{(i.e. floors)} it has (x)? This data (availabe on Moodle) was collected in 2018-2019 and recorded some information on different skyscrapers in NYC.(10 points)
...
d) Calculate a $95\%$ confidence interval for the model parameter $\beta_1$, the slope of the variable \textcolor{red}{"floors"}. How would you explain the meaning of this confidence interval, in the specific context of this dataset, to an architect who has never taken a statistics class?
...
g) Make a Normal probability plot to determine if the \textcolor{red}{response data looks like it comes} from a Normal distribution. Interpret your results. 


**Q 4)** 
\textcolor{red}{Let's consider the data set called} *msleep* \textcolor{red}{which is contained in the R package ggplot2 and is similar to the} *sleep* \textcolor{red}{data set that we used in HW 1. This }*msleep* \textcolor{red}{data set looks at the amount of time spent sleeping for different mammals and records other factors such as brain and body weight. For this problem, we are interested in the total amount of sleep an animal gets (}*sleep_total*\textcolor{red}{) as predicted by the total body weight of the animal (}*bodywt*\textcolor{red}{).} (4 points)


a) \textcolor{red}{There is a qualitative variable named} *order* \textcolor{red}{in this data set.} Fit two separate linear regression models for the animals of *order* "Carnivora" and of *order* "Primates". 


b) What is the estimate of the variance of the errors for each of these two models? 


c) Do you think We could fit a single linear regression model to \textcolor{red}{both} *orders* \textcolor{red}{Carnivora and Primates}? Justify your answer with \textcolor{red}{1-2} sentences and (possibly) a supporting plot.
