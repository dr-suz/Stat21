...
(a)  The output gives the fitted prediction model as $\hat{MMSE} = -0.59 + 2.32 APC - 1.85 TypeDLB/AD - 0.97APC \cdot TypeDLB/AD$. Thus when `Type` is `DLB`, the prediction model is $\hat{MMSE} = -0.59 + 2.32APC$, and when Type is DLB/AD, the prediction model is $\hat{MMSE} = -2.44 + 1.35APC$. 
