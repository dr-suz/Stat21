...
```{r}
regmod <- lm(height_meters ~ floors, data = skyscraper_data)

ggplot(skyscraper_data, aes(x=floors, y=height_meters)) +
  geom_point() 
```

**Solution:** 

(a)  `r regmod$coefficients` (must be written in proper form with all variables defined for full credit)

(b)  `r summary(regmod)$sigma` meters (must include units for full credit) 

(c) `r summary(regmod)$r.squared` or `r summary(regmod)$adj.r.squared`
...
```{r}
skyscraper_full <- skyscraper_data %>% mutate(resids = rstandard(regmod),
                                              fits = regmod$fitted.values)
ggplot(skyscraper_full, aes(x=fits, y=resids)) + 
  geom_point() + 
  geom_hline(yintercept=0) + 
  labs(title="Standardized residuals vs fitted values", x="Fitted values", y="Residuals")

ggplot(skyscraper_full, aes(sample=resids)) +
  stat_qq() + 
  stat_qq_line() + 
  labs(title="Normal quantile plot of standardized residuals", x="Theoretical quantiles", y="Observed residual quantiles")
...
**Solution:** Use code from Week 5 worksheet to create this plot 
